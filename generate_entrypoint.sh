...
sanity_options = [opt for opt in sanity_options if opt["long"] not in {"--max-workers", "--output-dir", "--stage1-only", "--stage2-only"}]
oxichecker_options = [opt for opt in oxichecker_options if opt["long"] not in {"--input", "--num_cores", "--batch_size", "--general-timeout", "--decomposition-timeout", "--obabel-timeout"}]
...
lines.append("  --run-sanity-only               Run only sanity runner (full Stage 1 + Stage 2)")
lines.append("  --run-stage1-only               Run only Stage 1 (LibCif fast filter)")
lines.append("  --run-stage2-only               Run only Stage 2 (no LibCif pre-filter)")
lines.append("  --run-oxichecker-only           Run only OxiChecker (legacy standalone)")
...
lines.append("      continue")
lines.append("      ;;")
lines.append("    --run-stage1-only)")
lines.append("      run_mode=\"stage1\"")
lines.append("      SANITY_ARGS+=(\"--stage1-only\")")
lines.append("      shift")
lines.append("      continue")
lines.append("      ;;")
lines.append("    --run-stage2-only)")
lines.append("      run_mode=\"stage2\"")
lines.append("      SANITY_ARGS+=(\"--stage2-only\")")
lines.append("      shift")
lines.append("      continue")
lines.append("      ;;")
...
lines.append('if [[ "$run_mode" == "sanity" || "$run_mode" == "stage1" || "$run_mode" == "stage2" || "$run_mode" == "all" ]]; then')
...
lines.append('fi')
lines.append('')
lines.append('if [[ "$run_mode" == "postprocess" || "$run_mode" == "all" || "$run_mode" == "stage1" || "$run_mode" == "stage2" ]]; then')
...
lines.append('    output_specified_post=false')
lines.append('    for i in "${!POST_ARGS[@]}"; do')
lines.append('      if [[ "${POST_ARGS[$i]}" == "--output" ]]; then')
lines.append('        output_specified_post=true')
lines.append('        break')
lines.append('      fi')
lines.append('    done')
lines.append('    if [[ "$output_specified_post" == false ]]; then')
lines.append('      POST_ARGS+=("--output" "$output_dir/sanity_results.csv")')
lines.append('    fi')
lines.append('  fi')
lines.append('  ')
lines.append('  if [[ -n "$n_jobs_value" ]]; then')
...
lines.append('  "stage1"|"stage2")')
lines.append('    mode_label="Stage 1" && [[ "$run_mode" == "stage2" ]] && mode_label="Stage 2"')
lines.append('    echo "=== Running $mode_label Only ==="')
lines.append('    if ! "$SANITY_BIN" "${SANITY_ARGS[@]}" "${SANITY_POSITIONALS[@]}"; then')
lines.append('      exit $?')
lines.append('    fi')
lines.append('    echo "=== Postprocessing ==="')
