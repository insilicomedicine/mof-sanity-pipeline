...
"""Basic sanity checks for Frames."""
import os
import warnings
...
from .checks.oms import FrameOMS
...
from .structure_graph_creator import get_structure_graph

__version__ = get_version()

__all__ = ["__version__", "FrameChecker", "DESCRIPTORS"]
...
class FrameChecker:
    """FrameChecker performs basic sanity checks for Frames."""
    ...
    def __init__(
        ...
        """Construct a FrameChecker instance.
        ...
        self._checks = {
            ...
            "no_overcoordinated_carbon": OverCoordinatedCarbonCheck.from_framechecker(self),
            "no_overcoordinated_hydrogen": OverCoordinatedHydrogenCheck.from_framechecker(self),
            "no_overcoordinated_nitrogen": OverCoordinatedNitrogenCheck.from_framechecker(self),
            "no_undercoordinated_rare_earth": UnderCoordinatedRareEarthCheck.from_framechecker(self),
            "no_undercoordinated_alkali_alkaline": UnderCoordinatedAlkaliAlkaline.from_framechecker(
                self
            ),
            "no_geometrically_exposed_metal": GeometricallyExposedMetal.from_framechecker(self),
            "no_floating_molecule": FloatingSolventCheck.from_framechecker(self),
            "no_oms": FrameOMS.from_framechecker(self),
            "no_false_terminal_oxo": FalseOxoCheck.from_framechecker(self),
        }
    ...
    @property
    def name(self) -> str:
        """Return filename stem if the FrameChecker instance was created based on a file."""
        return self._name
    ...
    def is_porous(self) -> Union[bool, None]:
        """Return True if the Frame is porous according to the CoRE-Frame definition.
    ...
    def graph(self) -> StructureGraph:
        ...
        #    self._graph = get_structure_graph(self.structure, tol=self.tol, min_bond_distance=self.min_bond_distance)
        #    self._nx_graph = construct_clean_graph(self._graph)
        return self._graph
    ...
    def get_frame_descriptors(self, descriptors=None) -> OrderedDict:
