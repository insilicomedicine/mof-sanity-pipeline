...
import structuregraph_helpers.analysis as sg_analysis
from pymatgen.analysis.local_env import JmolNN
from typing import TYPE_CHECKING, Literal, NamedTuple, get_args
import numpy as np
from ruamel.yaml import YAML
from pymatgen.core import Structure
import pymatgen.analysis as pymanalysis
import os
...
MODULE_DIR = pymanalysis.__path__[0]
...
class JmolNN_update(JmolNN):
    def __init__(
        ...
        self.max_rad = max([bond for bonds in self.bonds.values() for bond in bonds.values()]) + self.tol
        self.structure_get_neighbors = self.structure_to_work_with.get_all_neighbors(self.max_rad)
...
def get_structure_graph(structure: Structure,tol, min_bond_distance) -> StructureGraph:
    """Get a structure graph for a structure."""
    sg = JmolNN_update(structure, tol=tol, min_bond_distance=min_bond_distance).get_bonded_structure(structure)
